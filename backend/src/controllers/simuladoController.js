...
export const finalizarSimulado = async (req, res) => {
    try {
        const simulado = await simuladoService.finalizarSimulado(req.params.usuarioSimuladoId);
        res.status(200).json(simulado);
    } catch (error) {
        res.status(500).json({ message: "Erro", error });
    }
...
export const getSimuladosUsuario = async (req, res) => {
    try {
        const simulados = await simuladoService.getSimuladosUsuario(req.params.usuarioId);
...
export const calcularNota = async (req, res) => {
  try {
    const resultado = await simuladoService.calcularNota(req.params.usuarioSimuladoId);
...
export const getRelatorio = async (req, res) => {
    try {
        const relatorio = await simuladoService.getRelatorio(req.params.usuarioSimuladoId);
